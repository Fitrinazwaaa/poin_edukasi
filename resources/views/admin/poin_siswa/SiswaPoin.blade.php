...
<head>
    ...
    <style>
        /* Popover container */
        .popover {
            background-color: #f8f9fa; /* Warna latar belakang popover */
            border: 1px solid #ced4da; /* Warna border */
            border-radius: 8px; /* Border melengkung */
            box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.1); /* Shadow untuk kesan 3D */
        ...
        .popover-header {
            ...
            font-size: 16px; /* Ukuran font header */
            font-weight: bold;
            border-bottom: 1px solid #0056b3; /* Warna border bawah */
            border-radius: 8px 8px 0 0; /* Hanya melengkung di atas */
            padding: 10px;
        }

        /* Popover body */
        .popover-body {
            color: #495057; /* Warna teks body */
...
<body>
                            ...
                            <div class="modal-body" style="background-color: #e7f4ff;">
                                <div class="container">
                                    <form action="{{ route('StoreNamaPoinSiswa') }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="form-row">
                                            <label for="tingkatan" class="form-label" style="margin-right: 73px;">Kelas</label>
                                        ...
                                        <div class="form-row" id="foto_input_row" style="display: none;">
                                            <label for="foto" class="form-label">Unggah atau Ambil Foto</label>
                                            <input type="file" name="foto" id="foto" class="form-control" accept="image/*" capture="user">
                                        </div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <button class="add-btn" onclick="window.location.href='{{ route('viewSiswaDetail', ['nis' => $siswa->nis]) }}';">Lihat Detail</button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
    ...
    @elseif (Auth::user()->role == 'user4')
        <div class="button-container" tabindex="0">
            @foreach ($jumlahNotifikasi as $index => $jumlah)
                @php
                    $peringatan = match($index) {
                        0 => $poinPeringatan1->peringatan ?? 'Tidak Diketahui',
                ...
                @endphp
                <button 
                    class="notification-btn popover-btn" 
                    type="button"
                    tabindex="0" 
                    data-bs-toggle="popover" 
                    data-bs-trigger="hover focus" 
                    data-bs-title="Peringatan Akses"
                    data-bs-content="Akun Anda tidak bisa membuka halaman ini." 
                    disabled>
                    {{ $peringatan }}
                    <span class="notification-count">{{ $jumlah }}</span>
                </button>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const popoverList = popoverTriggerList.map(function (popoverTriggerEl) {
                    return new bootstrap.Popover(popoverTriggerEl, {
                        html: true, // Mengizinkan HTML di dalam popover
                        placement: 'auto', // Posisi popover otomatis
                    });
                });
            });
        </script>
